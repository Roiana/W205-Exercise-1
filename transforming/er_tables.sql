--Creating an entity from the raw data contained in hosp_gen_info
DROP TABLE ER_hosp_gen;
CREATE TABLE ER_hosp_gen AS
SELECT provider_id, hospital_name, state
FROM hosp_gen_info;

--Creating an entity from the raw data contained in time_effec_info
...
--Creating an entity from the raw data contained in read_deaths_info
DROP TABLE ER_read_deaths;
CREATE TABLE ER_read_deaths AS
SELECT provider_id, hospital_name, state, score
FROM read_deaths_info;

--Creating an entity from the raw data contained in surv_resp_info. Transforming the variables into percent form so that they can be calculated on, as they were in string format before.
